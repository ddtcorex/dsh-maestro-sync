...
- `lib/` — build output (gitignored). Generated by `pnpm build`; do not hand-edit.
- `tests/*.spec.ts` — vitest suites (host, client and rehearsal), incl. `hermetic-rehearsal.spec.ts`.
...
`pnpm build` is the required gate after any source change; `lib/` is build
output (gitignored), so a change is incomplete until the build refreshes it
locally — a `link:` install loads `lib/` from disk, and a release build comes
from CI, never from the working tree. The test script forces `NODE_ENV=test`
because an ambient `NODE_ENV=production` shell breaks the jsdom/react client
tests.
