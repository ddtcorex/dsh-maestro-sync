...
describe('snapshot', () => {
  ...
  it('creates snapshot for eligible memory path', async () => {
    const snap = await snapshotFile('dsh-maestro-memory/daily/2026-08-29.md', Buffer.from('content'));
    expect(snap.path).toBe('dsh-maestro-memory/daily/2026-08-29.md');
  ...
  it('rejects ineligible path', async () => {
    ...
    expect(() => snapshotFile('dsh-maestro-memory/a\u0000.md', Buffer.from(''))).toThrow();
  });

  it('identifies jsonl and session kinds via kindForPath', async () => {
    expect(kindForPath('dsh-maestro-memory/SUGGESTIONS.jsonl')).toBe('jsonl');
    expect(kindForPath('dsh-maestro-memory/daily/2026-08-29.md')).toBe('memory');
    expect(kindForPath('sessions/abc123/def456/session.jsonl.zstd')).toBe('session');
    const j = await snapshotFile('dsh-maestro-memory/SUGGESTIONS.jsonl', Buffer.from('[]'));
  ...
  it('collects only eligible files from a directory', async () => {
    ...
    try {
      const memDir = path.join(tmp, 'dsh-maestro-memory', 'daily');
      fs.mkdirSync(memDir, { recursive: true });
      fs.writeFileSync(path.join(memDir, '2026-08-29.md'), 'hello');
      fs.writeFileSync(path.join(tmp, 'dsh-maestro-memory', 'SUGGESTIONS.jsonl'), '{"a":1}\n');
  ...
  it('snapshotFromMap hashes validated paths only', async () => {
    ...
    expect(snaps.map((s) => s.path)).toEqual(['dsh-maestro-memory/daily/2026-08-29.md']);
  });
});
