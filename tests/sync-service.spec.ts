...
describe('SyncService', () => {
  ...
  it('statusPage paginates with a cursor and bounded page size', async () => {
    ...
    try {
      fs.mkdirSync(path.join(localRoot, 'dsh-maestro-memory', 'daily'), { recursive: true });
      for (let i = 0; i < 12; i++) fs.writeFileSync(path.join(localRoot, `dsh-maestro-memory/daily/2026-08-01-${String(i).padStart(2, '0')}.md`), `day ${i}\n`);
  ...
  it('statusPage buckets are action-based: remoteOnly includes both-side content differences', async () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(localRoot, 'dsh-maestro-memory/daily/2026-09-01.md'), 'local-day\n');
      // remote-only path (copy candidate)
      const fake = createFakeRemote(
        new Map<string, Buffer>([
          ['dsh-maestro-memory/daily/2026-09-01.md', Buffer.from('remote-day\n')],
          ['dsh-maestro-memory/daily/2026-09-02.md', Buffer.from('remote-new-day\n')],
        ]),
      );
      const svc = new SyncService({
        localDsh: localRoot,
      ...
      expect(page.files).toContain('dsh-maestro-memory/daily/2026-09-02.md');
      expect(page.files).toContain('dsh-maestro-memory/daily/2026-09-01.md');
      ...
      expect(localPage.files).toContain('dsh-maestro-memory/daily/2026-09-01.md');
    } finally {
      cleanup();
    }
  });
  ...
  it('stages only change candidates: identical bytes are never transferred', async () => {
    ...
    try {
      ...
      expect(preview.actions.find((a: any) => a.path === 'dsh-maestro-memory/shared.md')!.action).toBe('skip');
      expect(preview.actions.find((a: any) => a.path === 'dsh-maestro-memory/daily/2026-08-29.md')!.action).toBe('copy');
      // only the divergent file was staged
      expect(fake.calls.stage.length).toBe(1);
      expect(fake.calls.stage[0]!.paths).toEqual(['dsh-maestro-memory/daily/2026-08-29.md']);
    } finally {
      cleanup();
    }
  });
  ...
  it('eligibility: only memory md, SUGGESTIONS.jsonl and session zstd are snapshotted', async () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(localRoot, 'dsh-maestro-memory', 'SUGGESTIONS.jsonl'), '{"s":1}\n');
      fs.writeFileSync(path.join(localRoot, SESSION), 'not-real-zstd-bytes');
      // excluded files exist but must never be read/hashed
      fs.writeFileSync(path.join(localRoot, 'dsh-maestro-memory', 'daily', '2026-08-29.md.bak.1720000000.aa'), 'backup bytes');
      ...
      expect(paths).toContain('dsh-maestro-memory/daily/2026-08-29.md');
      expect(paths).toContain('dsh-maestro-memory/SUGGESTIONS.jsonl');
  ...
  it('preview uses the remote manifest and the fingerprint cache: unchanged files are not re-hashed, only changed remote files are staged', async () => {
    ...
    try {
      fs.mkdirSync(path.join(localRoot, 'dsh-maestro-memory', 'daily'), { recursive: true });
      fs.writeFileSync(path.join(localRoot, 'dsh-maestro-memory/daily/same.md'), 'same-bytes');
      fs.writeFileSync(path.join(localRoot, 'dsh-maestro-memory/daily/new.md'), 'local-new');
      const fake = createFakeRemote(new Map([
        ['dsh-maestro-memory/daily/same.md', Buffer.from('same-bytes')],
      ...
      const remoteAct = preview.actions.find((a: any) => a.path === 'dsh-maestro-memory/daily/remote.md');
      expect(remoteAct!.action).toBe('copy');             // remote-only → pull copy
      const newAct = preview.actions.find((a: any) => a.path === 'dsh-maestro-memory/daily/new.md');
      ...
      expect(stagedPaths).toContain('dsh-maestro-memory/daily/remote.md');
      expect(stagedPaths).not.toContain('dsh-maestro-memory/daily/new.md');
      expect(stagedPaths).not.toContain('dsh-maestro-memory/daily/same.md');
    } finally {
      cleanup();
    }
  });
