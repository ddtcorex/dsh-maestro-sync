...
4. Open the project in your editor. Host logic lives in `src/host/`, client UI in `src/client/`, tests in `tests/`. `lib/` is build output (gitignored) — never hand-edit it; run `pnpm build` after a source change.
